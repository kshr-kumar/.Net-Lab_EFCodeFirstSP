    ...
    public class AppDbContextProcedures
    {
        ...
        public Employee UspGetEmployee(int EmployeeId)
        {
            ...
            { 
                ...
                command.CommandType = CommandType.StoredProcedure; 
                var parameter = new SqlParameter("EmployeeId", EmployeeId); command.Parameters.Add(parameter); 
                _context.Database.OpenConnection(); 
                using (var reader = command.ExecuteReader()) 
                {
                    while (reader.Read()) 
        ...
        public bool UspAddEmployee(Employee employee)
        {
            ...
            using (var command = _context.Database.GetDbConnection().CreateCommand())
            {
                ...
                var paramDepartmentId = new SqlParameter("DepartmentId", employee.DepartmentId); //output parameter
